...
%(common_options)s
    ...
    By default, <corpus> must be either a valid XML file (mwetoolkit-corpus.dtd)
    or the path to a `.info` file generated by index.py.
...
with tempfile.NamedTemporaryFile( prefix=TEMP_PREFIX,
                                  dir=TEMP_FOLDER ) as temp_fh:
