...
-p <patterns.xml> OR --patterns <patterns.xml>
    The patterns to extract, valid XML (mwttoolkit-patterns.dtd)
...
-s OR --surface
    ...
    The <corpus.xml> file must be valid XML (mwttoolkit-corpus.dtd). You must 
...
def create_patterns_file( ngram_range ) :
    """
        Create an artificial list of MWT patterns in which all the parts of
...
def print_candidates( temp_file, corpus_name ) :
    """
        Prints a XML file (mwttoolkit-candidates.dtd) from a temporary 
